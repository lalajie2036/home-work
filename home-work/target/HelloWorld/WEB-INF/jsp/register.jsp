...
<div class="login-main">
    ...
    <!-- 表单选项 -->
    <form class="layui-form" action="${pageContext.request.contextPath}/library/submitAddReader.action" method="post">
        ...
        <hr style="width: 85%" />
        <p style="width: 85%"><a href="${pageContext.request.contextPath}/toLogin.htm" class="fl">已有账号？立即登录</a></p>
    </form>
</div>
...
<script type="text/javascript">
    layui.use(['form','jquery','layer'], function () {
        ...
        $('#user').blur(function() {
            ...
            //alert(user);
            $.ajax({
                url:'${pageContext.request.contextPath}/checkReader',    //注册时查询是否存在该用户
                type:'post',
                dataType:'json',
                data:{reader_id:user},
                //验证用户名是否可用
        ...
        // 为密码添加正则验证
        $('#pwd').blur(function() {
            var reg = /^[\w]{1,12}$/;   //匹配1-12个非[a-zA-Z0-9]，也就是报告特殊字符
        ...
        form.on('submit(sub)', function(data) {
            ...
            $.ajax({
                url:'${pageContext.request.contextPath}/submitAddReader',
                data:data.field,
                dataType:'json',
                type:'post',
                success:function(data){
