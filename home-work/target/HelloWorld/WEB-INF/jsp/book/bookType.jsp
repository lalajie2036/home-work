    ...
    <script type="text/javascript">
        layui.use(['laypage', 'layer', 'table', 'element','jquery'], function(){
            ...
            table.render({
                elem: '#demo'
                ,height: 550
                ,url: '${pageContext.request.contextPath}/type/bookTypeList' //数据接口
                ,title: '图书表'
                ,cols: [[ //表头
            ...
            function editBookType(data1){
                layer.prompt({title: '修改类别',value:data1.cname, formType: 2}, function(text, index){
                    layer.close(index);
                    $.ajax({
                        url:'${pageContext.request.contextPath}/type/editBookType',
            ...
            function del(cid,obj,index){
                ...
                $.ajax({
                    url:'${pageContext.request.contextPath}/type/delBookType.do?cid='+cid,
                    dataType:'json',
                    type:'post',
                    success:function (data) {
                        if (data.success){
        ...
        function addType(){
            layui.use(['laypage', 'layer', 'table', 'element','jquery'], function(){
                ...
                layer.prompt({title: '添加类别', formType: 2}, function(text, index){
                    layer.close(index);
                    $.ajax({
                        url:'${pageContext.request.contextPath}/type/addBookType.do',
                        data:{'cname':text},
                        dataType:'json',
                        type:'post',
                        success:function (data) {
